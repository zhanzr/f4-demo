...
static uint8_t TEST_MEMORY write_buffer[SPI_FLASH_TEST_SIZE];
static uint8_t TEST_MEMORY read_buffer[SPI_FLASH_TEST_SIZE];
...
int main(void)
{
    ...
    printf("\r\n==== fire-f429 SPI flash test ====\r\n");
    printf("W25Q128FVSG JEDEC ID: 0x%06lX, SPI5 clock: 45 MHz\r\n",
    ...
    printf("Erase 64 KiB: %lu cycles\r\n", (unsigned long)erase_cycles);
        ...
        printf("Program 64 KiB: %lu cycles, %lu.%03lu KiB/s\r\n",
           (unsigned long)program_cycles,
            (unsigned long)(program_rate / 1000U),
            (unsigned long)(program_rate % 1000U));
        printf("Read 64 KiB: %lu cycles, %lu.%03lu KiB/s\r\n",
