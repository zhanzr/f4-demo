...
#define SDRAM_WORDS      (8UL * 1024UL * 1024UL / sizeof(uint16_t))
...
int main(void)
{
    ...
    printf("Write: %lu cycles, %lu.%03lu MiB/s\r\n", (unsigned long)write_cycles,
           (unsigned long)((8UL * SystemCoreClock) / write_cycles),
           (unsigned long)(((8UL * SystemCoreClock) % write_cycles) * 1000UL / write_cycles));
    printf("Read:  %lu cycles, %lu.%03lu MiB/s\r\n", (unsigned long)read_cycles,
           (unsigned long)((8UL * SystemCoreClock) / read_cycles),
           (unsigned long)(((8UL * SystemCoreClock) % read_cycles) * 1000UL / read_cycles));
