...
volatile uint8_t OV5640_FrameState = 0;   /* set by HAL_DCMI_FrameEventCallback */
...
void OV5640_DMA_Config(uint32_t DMA_Memory0BaseAddr,uint32_t DMA_BufferSize)
{
  ...
  HAL_DCMI_Start_DMA(&DCMI_Handle, DCMI_MODE_SNAPSHOT, (uint32_t)DMA_Memory0BaseAddr,DMA_BufferSize);

}

/* Resume the DCMI after a snapshot transfer (the HAL clears CAPTURE in
 * snapshot mode). Call before starting the next snapshot. */
 ...
 * app defines them - the vendor example defines both in stm32f4xx_it.c.
 *
 * Without these two the first DCMI VSYNC hangs the CPU; the DMA (hardware
 * double-buffer) then only alternates its first two buffers, so only the
 * UPPER HALF of the framebuffer is captured (the lower half stays the
 * layer-0 blue fill) and the FPS counter freezes.
 *
 * The vendored HAL_DCMI_IRQHandler aborts the DMA on a sync/overflow error
 * (the vendor's older HAL 1.5.1 did not), so the error flags are cleared
 * here before the HAL sees them - same as the proven app/ov5640_to_lcd_armclang
 * driver. */
