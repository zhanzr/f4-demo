/*
 * WiFi AP scan for fire-f429 (EMW1062 / AP6181 / BCM43362 via SDIO1).
 *
 * Flow:
 ...
 *   startup_thread: tcpip_init() -> app_main() (scan.c: wwd_management_wifi_on + scan)
 *
...
extern void app_main(void);                    /* scan.c */
...
int main(void)
{
    ...
    BaseType_t ret = xTaskCreate((TaskFunction_t)startup_thread,
                                 "app_thread",
                                 APP_THREAD_STACKSIZE / sizeof(portSTACK_TYPE),
                                 NULL,
                                 (UBaseType_t)DEFAULT_THREAD_PRIO,
...
static void startup_thread(void *arg)
{
    ...
    /* Signal when LwIP has finished initialising, then run the scan app. */
