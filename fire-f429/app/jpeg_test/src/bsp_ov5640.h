...
The clone uses QVGA 320x240: the frame (153600 bytes = 38400 words) fits a
single DMA buffer (<= 0xFFFF words), so DCMI snapshot mode captures ONE
complete, aligned frame per transfer - the stable-capture path (the vendor's
800x480 is 192000 words, which forces the HAL's flaky double-buffer path). */
#define img_width  320
#define img_height 240

/* Snapshot capture buffer: one QVGA RGB565 frame (DMA2-accessible SRAM). */
extern uint8_t snap_buf[img_width * img_height * 2];
