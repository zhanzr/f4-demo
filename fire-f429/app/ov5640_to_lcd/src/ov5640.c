...
/* Display-oriented consumer: returns the NEWEST complete RGB565 frame in
 * the ring (or 0 if the DMA has not finished even one frame yet).
 *
 * Unlike OV5640_GetFrame() (oldest-first, for recorders/HTTP), this never
 * stalls on the wrap-resync: the camera may produce frames faster than the
 * consumer displays them, and the newest complete frame is always ready
 * (the slot before the one being written). The returned pointer stays
 * valid until the next call - the DMA needs 3 more frame periods (~50 ms)
 * to wrap around and overwrite it, longer than any blit in practice. */
int OV5640_GetLatestFrame(const uint8_t **frame)
{
    uint32_t ndtr      = hdma_dcmi.Instance->NDTR;
    uint32_t write     = (RING_WORDS - ndtr) * 4U;
    uint32_t start_cur = write - (write % OV5640_FRAME_BYTES); /* being written */
    uint32_t pos;

    if (start_cur == 0U)
    {
        if (write < OV5640_FRAME_BYTES)
        {
            return 0;   /* still capturing the very first frame */
        }
        pos = OV5640_FRAME_BUF_SIZE - OV5640_FRAME_BYTES;   /* last slot */
    }
    else
    {
        pos = start_cur - OV5640_FRAME_BYTES;               /* slot before */
    }

    *frame = &frame_ring[pos];
    g_last_frame_tick = HAL_GetTick();
    g_frame_count++;
    return 1;
}
