/*
 * WiFi HTTP server for fire-f429 (EMW1062 / AP6181 / BCM43362 via SDIO1).
 *
 * Flow:
 ...
 *                   -> http_server_init() (lwIP raw-API HTTP server on :80)
 *
...
extern void http_server_init(void);            /* src/http_server.c */
...
static void startup_thread(void *arg)
{
    ...
    connect_main();          /* joins the AP + DHCP; blocks until bound */
    http_server_init();      /* raw-API HTTP server on port 80          */
}
