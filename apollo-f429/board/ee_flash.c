...
void HAL_I2C_MspInit(I2C_HandleTypeDef *hi2c)
{
    ...
    if (hi2c->Instance == EEPROM_I2C)
    {
        EEPROM_GPIO_CLK_EN;
        EEPROM_I2C_CLK_EN;

        /* PH4 = I2C2_SCL, PH5 = I2C2_SDA (AF4), open-drain + pull-ups. */
        gpio.Pin = EEPROM_SCL_Pin | EEPROM_SDA_Pin;
        gpio.Mode = GPIO_MODE_AF_OD;
        gpio.Pull = GPIO_PULLUP;
        gpio.Speed = GPIO_SPEED_FREQ_VERY_HIGH;
        gpio.Alternate = EEPROM_GPIO_AF;
        HAL_GPIO_Init(EEPROM_GPIO_Port, &gpio);
    }
}

void EE_Flash_Init(void)
{
    ee_i2c.Instance = EEPROM_I2C;
