/*
  ...
  touch/实验30. Three things run, top to bottom:

    1. LCD init + ID read, then the vendor's TFTLCD TEST loop (12 background
       colors cycling every second with a text banner - mirrored from the
       tft_lcd_test vendor main).
    2. The test patterns from the st7789_md169 reference project: an info page,
       the TEST_STAND screens (DispFrame / DispGrayHor16 / DispBand /
       DispColor), an HSV gradient sweep, an LED check, all with a live FPS
       counter.
    3. Touch: TP_Init() (auto-detect + first-boot 4-point calibration stored on
       the AT24C02 EEPROM), then while the patterns run a touch scan draws a
       big point wherever the panel is pressed, and the "RST" corner clears.
...
int main(void)
{
    ...
    printf("\r\n==== apollo-f429 (STM32F429IGT6) lcd_touch_test @ %lu MHz ====\r\n",
