...
~2× (measured elsewhere in this repo: 770,713 vs 351,370 Dhrystones/s — the
same mechanism applies here). The result still *looks* correct (the hoisted
code still runs once, so the final values match), which is exactly why this
number must never be quoted. This is a **known, documented weakness of
Dhrystone itself**, not a bug in our toolchain or setup.
...
## Measured evidence (a 168 MHz STM32F407, `-Ofast` — same mechanism on apollo-f429)
