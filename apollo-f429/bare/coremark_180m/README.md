# CoreMark 1.0.1 @ 180 MHz — apollo-f429 (STM32F429IGT6)

CoreMark 1.0.1 (EEMBC, `coremark_1_0_1/`), **10,000 iterations**, on the
**apollo-f429** board (STM32F429IGT6, bare metal) at **180 MHz** (HSE 25 MHz →
...
| --------- | ------| ------------ | -------- |
| GCC | `-Ofast -ffp-contract=fast -funroll-all-loops` (default) | **495.47** | 20.18 |
| GCC + LTO | default `+ -DSTM32_LTO=ON` | 490.70 | 20.38 |
| ARMCLANG (Keil AC6) | `-Omax -fno-lto` | **599.16** | 16.69 |
| ST Arm clang | `-Ofast -ffp-contract=fast -funroll-loops` | 447.99 | 22.32 |
...
**ARMCLANG at `-Omax` is the fastest** at 599.16 iterations/s. Bare `-Omax`
makes armclang emit LLVM LTO objects that GNU ld cannot link — see the
per-toolchain recipes below. GCC `-flto` adds nothing (490.70 vs 495.47,
...
- **ARMCLANG (Keil AC6): `-Omax -fno-lto`** — 599.16 it/s. Pass it as a
...
- **GCC:** `-Ofast -ffp-contract=fast -funroll-all-loops` (default) →
  495.47 it/s. `-DSTM32_LTO=ON` is unchanged (490.70).
...
```
Total time (secs) = 20.183000
Iterations/Sec   = 495.466482
Iterations/Sec   = 495.466482
```
