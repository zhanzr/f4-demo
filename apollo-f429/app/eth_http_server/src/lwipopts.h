...
/* 80 KB: large enough for tcp_write(TCP_WRITE_FLAG_COPY) bursts of the
 * gzip page + embedded images (queued in segments from this heap). */
...
/* Large send buffer so the gzip page + JPEG board photos fit in one
 * tcp_write burst. */
 ...
 * for debugging (host adapter is 192.168.5.99). */
#define LWIP_DHCP               0
