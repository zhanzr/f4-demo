/**
  ...
  *          apollo-f429 board (RMII + LAN8720A PHY).
  *
  * Based on the STM32F769I-Discovery eth_http ethernetif.c, re-targeted to
  * the STM32F429 HAL + the apollo-f429 LAN8720A PHY pins.
  *
  * RMII pins (apollo-f429, same 10-wire set as the vendored 实验58 demo):
  *   RMII_REF_CLK -> PA1, MDIO -> PA2, MDC -> PC1, CRS_DV -> PA7,
  *   RXD0 -> PC4, RXD1 -> PC5, TX_EN -> PB11, TXD0 -> PG13, TXD1 -> PG14
  *
  * Vendor note (实验58): MDIO shares USART2_TX (PA2) and RMII_TX_EN shares
  * USART3_RX (PB11), so those UARTs must stay off while ETH is in use. The
  * LAN8720A ETH_RESET is driven from PCF8574 P7 (through Q1), so the PHY is
  * reset from lan8720a.c via myiic/PCF8574 before MDIO is touched.
  *
...
static void low_level_init(struct netif *netif)
{
  ...
  /* Initialize the LAN8720A ETH PHY (hardware reset via PCF8574 P7 first) */
...
*******************************************************************************/
/**
  * @brief  Initializes the ETH MSP: clocks + RMII GPIOs (apollo-f429).
  */
