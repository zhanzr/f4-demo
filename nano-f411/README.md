...
| `app/ili9163c_md144_128x128` | **ILI9163C 1.44" 128x128** LCD (MD144 connector, **no D/C pin**: 3-wire serial, 9-bit frames; **backlight hardwired on**) - dual-bus demo (soft bit-bang + HW SPI1 with the 9-bit stream packed into 16-bit words), display rotated 180 deg via MADCTL (SCL=PA5 SDA=PA7 RES=PA6 CS=PB8) |
| `app/st7365_md350_320x480` | **ST7365P 3.5" 320x480** LCD (MD350, MADCTL 0x48 portrait) - dual-bus demo (soft bit-bang + HW SPI1 @ 50 MHz) (SCL=PA5 SDA=PA7 **RES=PA3** DC=PA4 CS=PB8 BL=PB9; **MISO=PA6** unused by the TX-only driver) |
| `app/nv3030b_md183_240x284_cst816d` | **NV3030B 1.83" 240x284** LCD (wrapped-command SPI: CS↓ `02 00 cmd 00` then data; **DC not used**) + **CST816D** touch (bit-banged I2C PA2/PA3) with serial printout (SCL=PA5 SDA=PA7 CS=PA4) - dual-bus demo |
