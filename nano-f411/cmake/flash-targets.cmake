# Shared flashing / SWV targets for the STM32F411CEU6 "nano-f411" board, programmed
...
set(OPENOCD_CFG "${CMAKE_CURRENT_LIST_DIR}/openocd_stm32f411ce.cfg")
...
if(PROBE_RS)
    add_custom_target(flash
        COMMAND "${PROBE_RS}" download ${PROBE_ARGS}
                    --chip STM32F411CE --protocol swd
                    --binary-format hex --verify --reset --non-interactive
                    --disable-progressbars "${BIN_HEX}"
        DEPENDS ${BIN_HEX}
        COMMENT "Flashing ${PROJECT_NAME}.hex to STM32F411CEU6 via probe-rs (ST-Link, SWD) ..."
        USES_TERMINAL)
else()
    add_custom_target(flash
...
if(OPENOCD)
    add_custom_target(flash-ocd
        ...
        DEPENDS ${BIN_HEX}
        COMMENT "Flashing ${PROJECT_NAME}.hex to STM32F411CEU6 via OpenOCD (ST-Link, SWD) ..."
        USES_TERMINAL)
else()
    add_custom_target(flash-ocd
