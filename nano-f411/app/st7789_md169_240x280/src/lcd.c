/*
  lcd.c - ST7789V/T3 1.69" 240x280 LCD driver (nano-f411 port, MD169 module).
  Same driver skeleton as the st7735_md144_128x128 baseline (nano-f411,
  bit-banged 4-wire SPI + HW SPI1), with the ST7789V init sequence and the
  MD169 geometry from the c542 board_t1 st7789 bsp:
    - 240x280 glass inside the ST7789's 240x320 GRAM: ROW_Pre = 20
      (glass covers GRAM rows 20..299 - verified on the c542 board; 0 shows
      a bottom gap, 40 a top gap),
    - MADCTL 0x00, IPS inversion (INVON 0x21),
    - ST7789 power/porch/gamma init.
  Wiring is the same as the st7735 project:
...
/* =====================================================================
   Init sequence - ST7789V/T3 (MD169), from the c542 board_t1 st7789 bsp
   (bsp/st7789/lcd.c, verified on the MD169 panel there): power/porch/
   gamma registers specific to the ST7789, IPS inversion (INVON), MADCTL
   0x00 and 16bpp COLMOD.
   ===================================================================== */
void LCD_IC_Init(void)
{
    ...
    WriteData(0x55);
    WriteComm(0x21);              /* INVON - IPS panel, inverted colors */

    WriteComm(0xB2);              /* PORCTRL */
    WriteData(0x0C); WriteData(0x0C); WriteData(0x00);
    WriteData(0x33); WriteData(0x33);

    WriteComm(0xBB);              /* VCOMS */
    WriteData(0x1F);

    WriteComm(0xC2);              /* VDVVRHEN */
    WriteData(0x01);
    ...
    WriteComm(0xC5);              /* VCOM offset */
    WriteData(0x00);

    WriteComm(0xC6);              /* FRCTRL2 */
    WriteData(0x0F);

    WriteComm(0xD0);              /* PWCTRL1 */
    WriteData(0xA4); WriteData(0xA1);

    WriteComm(0xE0);              /* PVGAMCTRL */
    WriteData(0xD0); WriteData(0x0A); WriteData(0x10); WriteData(0x0A);
    WriteData(0x0A); WriteData(0x26); WriteData(0x36); WriteData(0x34);
    WriteData(0x4D); WriteData(0x18); WriteData(0x13); WriteData(0x14);
    WriteData(0x2F); WriteData(0x34);

    WriteComm(0xE1);              /* NVGAMCTRL */
    WriteData(0xD0); WriteData(0x0A); WriteData(0x10); WriteData(0x0A);
    WriteData(0x09); WriteData(0x26); WriteData(0x36); WriteData(0x53);
    WriteData(0x4C); WriteData(0x18); WriteData(0x14); WriteData(0x14);
    WriteData(0x2F); WriteData(0x34);

    WriteComm(0x2A);              /* CASET 0..239 */
    WriteData(0x00); WriteData(0x00); WriteData(0x00); WriteData(0xEF);

    WriteComm(0x2B);              /* RASET (vendor; draw windows extend
                                     to ROW-1 via BlockWrite) */
    WriteData(0x00); WriteData(0x00); WriteData(0x00); WriteData(0xEF);
