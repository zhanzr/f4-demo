/*
  st7789_md169_240x280 main for the nano-f411 (STM32F411CEU6 @ 100 MHz).
  The st7735_md144_128x128 baseline, re-targeted to the MD169 module:
  ST7789V/T3 1.69" 240x280 IPS (init sequence + geometry ported from the
  c542 board_t1 st7789 bsp - 240x280 glass in the 240x320 GRAM at
  ROW_Pre = 20, MADCTL 0x00, INVON). Same wiring as the st7735 project.

  Pattern set (md130/md169 style), on BOTH drive methods:
  ...
  25 MHz (prescaler /4 - the ST7735-class panels' proven stable rate;
  /2 = 50 MHz is the F411 SPI1 max if a unit proves it can take it). The
  core stays at 100 MHz; USART1 (APB2) recomputes its baud from the live
...
#define SCREEN_H   LCD_Height    /* 280 */
 ...
 * keeps the compact lines of the st7735 baseline anyway. */
#define INFO_DY    13
...
/* --------------------------------------------------------------------- */
/* Vendor TEST_STAND screens (240x280).                                  */
...
/* Info page (ported from the md130 demo): compiler, build date, current
   frequency, driving method, the (swapped) IO map, backlight duty and the
   MCU unique device ID. `invert` swaps fg/bg (white background page).
   Keeps the compact lines of the st7735 baseline; no ADC block (this
   board's baseline has no adc_internal helper).                          */
...
int main(void)
{
    ...
    printf("\r\n==== nano-f411 (STM32F411CEU6) st7789_md169_240x280 @ %lu MHz ====\r\n",
           (unsigned long)(SystemCoreClock / 1000000UL));
    printf("ST7789V/T3 1.69\" 240x280 (MD169, IPS INVON, ROW_Pre=20), "
           "4-wire SPI:\r\n");
    printf("SCL=PA5 SDA=PA7 RES=PA6 DC=PA4 CS=PB8 BL=PB9(PWM 15%% soft / "
           "13%% hw)\r\n");
    printf("(SDA/RES swapped vs the vendor C8T6_md144_t1 wiring)\r\n");
    printf("Driving details from c542 st7789 bsp; baseline st7735_md144\r\n");
