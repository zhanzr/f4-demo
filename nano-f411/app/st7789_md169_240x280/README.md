# st7789_md169_240x280 - ST7789T3 1.69" 240x280 LCD on nano-f411

Drives the **MD169** 1.69" **240x280** IPS module (**ST7789T3** controller)
on the **nano-f411** board (STM32F411CEU6 @ 100 MHz) over a 4-wire SPI bus,
...
- **ST7789T3, IPS panel**: the init sequence enables color inversion
  (`INVON`, 0x21), sets MADCTL 0x00 and 16-bit COLMOD (0x3A = 0x55), then
  the ST7789 power/porch/gamma registers (0xB2/0xBB/0xC2/0xC3/0xC4/0xC5/
  0xC6/0xD0/0xE0/0xE1).
- **Geometry**: the ST7789 GRAM is 240x320 while the glass shows 240x280,
  so every draw window is offset by **`ROW_Pre = 20`** (the glass covers
  GRAM rows 20..299; 0 leaves a bottom gap, 40 a top gap).
...
67200 pixels, so the bit-banged pass runs at single-digit fps on the
gradient while the HW SPI1 pass stays smooth (~20 fps).
...
- `src/lcd.c` / `lcd.h` - ST7789T3 init + 240x280/ROW_Pre=20 geometry +
