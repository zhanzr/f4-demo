...
/* VREFINT typical value from the STM32F411 datasheet (1.18..1.24 V, typ 1.21). */
...
int main(void)
{
    ...
    while (1)
    {
        /* Basic operation: blink the single LED (PC13, low-active). */
        if (phase & 1)
        {
            LED_ON();
        }
        ...
        if (phase % 4 == 0)
        {
            ...
            if (now - last_report >= 1000)
            {
                ...
                /* VBAT voltage: on F411 the VBAT channel measures VBAT/4
                 * internally (1/4 divider ladder before IN18), so scale by 4. */
