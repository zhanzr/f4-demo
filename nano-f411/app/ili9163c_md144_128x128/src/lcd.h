/*
  lcd.h - ILI9163C 1.44" 128x128 LCD driver (nano-f411 port, MD144 module).
  Same drawing API as the other nano-f411 LCD projects (24-bit colors,
  lines, rectangles, circles, fills, buffer copy, ASCII text).
  Pins: SCL=PA5, SDA=PA7, RES=PA6, CS=PB8 (3-wire serial: no D/C pin on
  the module connector - the D/C bit is clocked in-protocol; the module's
  backlight is hardwired on-board).
  Panel geometry: ILI9163C 128x128, MADCTL 0xC8, windows at COL_Pre = 0,
  ROW_Pre = 0 (the init addresses columns 0..127 / pages 0..159 directly).
*/
...
/* ---- serial control pins (3-wire: D/C is clocked in-protocol) ---- */
