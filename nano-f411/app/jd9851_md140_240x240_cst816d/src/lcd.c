/*
  lcd.c - JD9851 1.4" 240x240 LCD driver (nano-f411 port, MD140 module).
  The JD9851 (ST7789-compatible claim) uses a plain DC-based 4-wire SPI
  protocol (vendor-verbatim driving): the command byte goes out with
  DC(RS)=PA6 low, every data byte with DC high.

  Wiring: SCL=PA5, SDA/MOSI=PA7, CS=PA4, DC=PA6. The module has no
  reset pin (the vendor sequence just settles CS) and no backlight pin
  (hardwired on-module).
...
void LCD_GPIOInit(void)
{
    ...
    g.Pin   = LCD_CS_Pin | LCD_RS_Pin;   /* PA4 = CS, PA6 = DC(RS) */
    HAL_GPIO_Init(GPIOA, &g);

    /* Idle level: CS high (deselected), DC high (data). */
    LCD_CS_SET;
    LCD_RS_SET;
}
...
/* =====================================================================
   Init sequence - JD9851 (vendored TK014F1828_1246 example, verbatim
   registers): password unlock (0xDF), page select (0xDE), panel power/
   DCDC/gamma/timing registers across PAGE0/PAGE2, sleep out (120 ms),
   oscillator + MIPI timing, INVON, COLMOD 16bpp, MADCTL 0x00, display
   on.
   ===================================================================== */
void LCD_IC_Init(void)
{
    ...
    WriteComm(0xB7);              /* VGMP/VGSP/VGMN/VGSN */
    WriteData(0x31);
    WriteData(0x88);
    WriteData(0x31);
    WriteData(0x10);
    ...
    WriteData(0x3F); WriteData(0x3C); WriteData(0x34); WriteData(0x2D);
    WriteData(0x2D); WriteData(0x2E); WriteData(0x2A); WriteData(0x2C);
    WriteData(0x2C); WriteData(0x2D); WriteData(0x2D); WriteData(0x2A);
    WriteData(0x2A); WriteData(0x2A); WriteData(0x28); WriteData(0x0E);
    WriteData(0x3F); WriteData(0x3C); WriteData(0x34); WriteData(0x2D);
    WriteData(0x2D); WriteData(0x2E); WriteData(0x2A); WriteData(0x2C);
    WriteData(0x2C); WriteData(0x2D); WriteData(0x2D); WriteData(0x2A);
    WriteData(0x2A); WriteData(0x2A); WriteData(0x28); WriteData(0x0E);

    WriteComm(0xB9);              /* POW_CTRL */
    WriteData(0x33);
    WriteData(0x28);
    WriteData(0xCC);

    WriteComm(0xBB);              /* DCDC_SEL */
    WriteData(0x06);
    WriteData(0x7A);
    WriteData(0x30);
    WriteData(0x30);
    ...
    WriteComm(0xBC);              /* VDDD_CTRL */
    WriteData(0x38);
    WriteData(0x3C);

    WriteComm(0xC0);              /* SETSTBA */
    WriteData(0x31);
    WriteData(0x20);

    WriteComm(0xC1);              /* SETPANEL (default) */
    WriteData(0x16);

    WriteComm(0xC3);              /* SETRGBCYC */
    WriteData(0x08);
    WriteData(0x00);
    WriteData(0x0A);
    WriteData(0x10);
    ...
    WriteComm(0xC4);              /* SETRGBCYC (default) */
    WriteData(0x00);
    WriteData(0xA0);
    WriteData(0x79);
    WriteData(0x0E);
    ...
    WriteComm(0xD7);              /* RAMCTRL (default) */
    WriteData(0x00);
    WriteData(0x00);
    ...
    WriteComm(0xC5);              /* OSCM: 10 MHz */
    WriteData(0x01);
    WriteData(0x00);
    WriteData(0x00);
    HAL_Delay(1);

    WriteComm(0xCA);              /* SETMIPI_2 */
    WriteData(0x10);
    WriteData(0x20);
    WriteData(0xF4);
    HAL_Delay(1);
 ...
 * sequence again - its password/page-select preamble restarts the
 * panel's register config, and the sleep out inside provides the
 * power-cycle-like reset. */
