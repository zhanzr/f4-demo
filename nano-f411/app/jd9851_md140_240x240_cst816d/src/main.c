/*
  jd9851_md140_240x240_cst816d main for the nano-f411
  (STM32F411CEU6, APB2 overridden to 100 MHz). MD140 module: JD9851
  240x240 IPS driven over its plain DC-based 4-wire SPI protocol
  (vendor-verbatim init from STM32_LCD_TK014F1828_1246_hardSPI_hal_
  captouch), on the HARDWARE SPI1 bus @ 50 MHz (APB2/2; lower rates
  via LCD_SPI1_PRESC), plus CST816D capacitive touch (I2C PA2/PA3)
  with the touch state printed on the serial console.
  ...
  Wiring: SCL=PA5, SDA/MOSI=PA7, CS=PA4, DC(RS)=PA6. The module has
  no reset pin (the vendor settles CS instead) and no backlight
  control pin (hardwired on-module).
...
static void info_demo(uint32_t ms, uint8_t invert)
{
    ...
    snprintf(buf, sizeof buf, "CS=PA4 DC=PA6");
    LCD_DisplayString(ix, (uint16_t)y, buf);  y += INFO_DY;
...
int main(void)
{
    ...
    printf("\r\n==== nano-f411 (STM32F411CEU6) jd9851_md140_240x240_cst816d @ %lu MHz ====\r\n",
           (unsigned long)(SystemCoreClock / 1000000UL));
    printf("JD9851 1.4\" 240x240 (DC-based 4-wire SPI, MADCTL 0x00):\r\n");
    printf("SCL=PA5 SDA=PA7 CS=PA4 DC=PA6\r\n");
    ...
    while (1)
    {
        ...
        banner_page("JD9851", "HW SPI1 test",
