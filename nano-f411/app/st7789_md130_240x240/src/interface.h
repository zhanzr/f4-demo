/*
  interface.h - low-level ST7789V bus primitives (nano-f411 port).
  One wiring (SCL=PA5, SDA=PA7), two drive methods selected at runtime:
    ...
    HW  : PA5/PA7 re-muxed to SPI1 AF5 (SCK / MOSI), mode 3 (CPOL=1
          ...
          APB2 = 100 MHz (project clock override) -> the default
          prescaler /4 gives 25 MHz; /2 = 50 MHz (F411 SPI1 max) and
          /8 = 12.5 MHz are available via LCD_SPI1_PRESC.

  Pin roles:
    SCL = PA5  (SPI clock, bit-banged or SPI1_SCK)
    SDA = PA7  (SPI MOSI, bit-banged or SPI1_MOSI)
