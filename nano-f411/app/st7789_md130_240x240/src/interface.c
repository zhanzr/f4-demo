/*
  interface.c - low-level ST7789V bus primitives (nano-f411 port).
  MERGE version: one wiring (SCL=PA5, SDA=PA7), two drive methods selected
  at runtime:

    SOFT: PA5/PA7 re-muxed to plain GPIO outputs and bit-banged (the
          proven path; DC selects command vs data, CS asserted per
          transfer).
    HW  : PA5/PA7 re-muxed to SPI1 AF5 (SCK/MOSI), master, simplex TX
          ...
          /2 = 50 MHz is the fastest the board can produce (F411 SPI1
          max); /8 = 12.5 MHz is the conservative fallback if a module
          shows corruption.
 ...
 * produce (F411 SPI1 max) - raise to /2 only if a module proves it can
 * take it, drop to /8 = 12.5 MHz if 25 MHz shows corruption. */
...
/* Re-apply the SPI1 config with the project-selected baud prescaler. */
void SPI_HW_SetSpeed(void)
{
    if (s_spi_ready == 0U)
    {
        return;
