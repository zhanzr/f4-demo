...
A minimal single-page web app for the STM32F407 "dev1-f407" board, plus a
host-side C backend that mirrors the API the board's embedded server will
implement. The web assets are bundled into C arrays by a build script
...
1. **LED control** - a single checkbox reflecting the board LED (PE13).
   Each click POSTs the new state to the backend (no page reload) and the
   checkbox re-syncs to the server's reply. Rapid clicks are coalesced.
2. **Sensors** - three canvas plots from the ADC1 internal channels:
   VREFINT / die temperature / VBAT. A dropdown picks the sample interval:
   ...
   timer and aborts any pending request). At most one request is in flight
   at a time, so the selected interval is always respected; on timeout/error
   the tick is skipped and the last value stays on screen.
3. **Board info** - architecture, LAN IP, public IP, geo location and weather
   (each falls back to "N/A" if unavailable), a manual *Refresh* button.
...
| `GET /api/leds`      | `{"leds":[0]}`                                  |
| `POST /api/leds`     | body `{"leds":[0]}` -> applied, `{"leds":[0]}` |
...
On the host the reference backend fetches the public IP / geo / weather over
plain HTTP (api.ipify.org, ip-api.com, wttr.in) - best-effort, so offline
they come back as `null` and the page shows "N/A". On the board those fields
would come from an HTTP client (+ TLS for HTTPS upstreams).
