...
/* 40 KB: enough for the page + JSON responses (no camera/JPEG frames on
 * this board, so no large tcp_write bursts). Fits the 128 KB SRAM. */
...
#define MEMP_NUM_TCP_SEG        48   /* >= TCP_SND_QUEUELEN for 32KB sndbuf */
 ...
 * TCP_SND_BUF = 32 KB = 32768 overflows to 0, so pcb->snd_buf = 0 and
 ...
 * TCP_RCV_SCALE 0 keeps a small receive window (TCP_WND < 32 KB needs no
 * scaling) while allowing the large 32 KB send window. */
#define LWIP_WND_SCALE          1
#define TCP_RCV_SCALE           0
/* Send buffer: 32 KB is plenty for the page + JSON (no camera/JPEG frames
 * on this board). */
