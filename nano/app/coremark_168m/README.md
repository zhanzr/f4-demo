# CoreMark 1.0.1 @ 168 MHz — nano (STM32F407VET6)

CoreMark 1.0.1 (EEMBC, `coremark_1_0_1/`), **10,000 iterations**, on the
**nano** board (STM32F407VET6) at **168 MHz** (HSE 8 MHz → PLL M=8 N=336 P=2
→ SYSCLK). Compiler-agnostic: the same sources build with
...
the dev1 board — ~0.3 %, noise — because CoreMark's per-run CRC forces the
work to execute, so LTO cannot cheat it the way it cheats Dhrystone; see
